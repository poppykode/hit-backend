...
def login(request):
    ...
    meal = Meal.objects.filter(user=token.user_id).exists()
    return Response({'token': token.key, 'user_id': token.user_id, 'course_id': qs.course.id, 'has_accomodation': acc, 'has_meal': meal}, status=HTTP_200_OK)
